...
import {fetchRestaurantsByAddress, fetchRestaurantsByGoogleAddress} from "../redux/actions/dataActions";
...
export default function CustomizedInputBase(props) {
    ...
    const getBrowserLocationGoogle = () => {
        navigator.geolocation.getCurrentPosition(
            function (position) {
                getUserAddressByGoogle(position.coords.latitude, position.coords.longitude);
            },
            function (error) {
                alert("The Locator was denied, Please add your address manually");
            }
    ...
    const handleSelectGoogle = async (value) => {
        if (value === "") localStorage.removeItem("location");
        else localStorage.setItem("location", value);
        setAddress(value);
        const results = await geocodeByAddress(value);
        const latlng = await getLatLng(results[0]);
        if (latlng) localStorage.setItem("latlng", `${latlng.lat}, ${latlng.lng}`);
        fetchRestByGoogleLocation(latlng);
    };
    ...
    const fetchRestByGoogleLocation = (latlng) => {
        console.log(latlng);
        dispatch(fetchRestaurantsByGoogleAddress(latlng.lat, latlng.lng));
        props.action(true);
    }
    ...
    const getUserAddressByGoogle = (lat, long) => {
        const latlng = {
            lat: lat,
            lng: long,
        };
    ...
    return (
        <Paper
            component="form"
            className={page === "home" ? classes.rootHome : page === "search" ? classes.rootSearch : classes.rootItems}
        >
                ...
                >
                    {({
                        ...
                        <>
                            ...
                            />
                            <div className={classes.results2}>
                                {loading ? <div>Getting Results...</div> : null}
            ...
            {page === "search" && (
                <>
                    ...
                    <IconButton
                        ...
                        className={classes.iconButton}
                        aria-label="directions"
                        onClick={getBrowserLocationGoogle}
                    >
                        <MyLocation/>
                    </IconButton>
                </>
