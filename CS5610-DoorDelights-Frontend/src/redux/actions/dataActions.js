...
export const fetchRestaurantsByGoogleAddress = (lat, lng) => (dispatch) => {
  dispatch({ type: LOADING_DATA });
  axios
    .get(`https://anywhere-cors-cs5610.herokuapp.com/https://maps.googleapis.com/maps/api/place/nearbysearch/json?location=`+`${[parseFloat(lat),parseFloat(lng)]}`+`&radius=4000&type=restaurant&keyword=cruise&key=AIzaSyAKQqq50lkrkea62KtAPTgHM_66NbsFvHQ`)
    .then((res) => {
      dispatch({
        type: SET_RESTAURANTS,
        payload: res.data,
...
export const fetchRestaurantFromGoogle = (place_id) => (dispatch) => {
  dispatch({ type: LOADING_DATA });
  axios
    .get(`https://anywhere-cors-cs5610.herokuapp.com/https://maps.googleapis.com/maps/api/place/details/json?place_id=${place_id}&key=AIzaSyAKQqq50lkrkea62KtAPTgHM_66NbsFvHQ`)
    .then((res) => {
      dispatch({
        type: SET_RESTAURANT,
        payload: res.data,
...
export const deleteRestaurant = (restData) => (dispatch) => {
  console.log("shreya: ", restData);
  axios
      .delete(`/delete-restaurant/${restData}`)
      .then((res) => {
        dispatch({
